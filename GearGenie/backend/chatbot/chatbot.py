...
def get_context_from_mongo(collection, query):
    """
...
def chat():
    """Main chat loop."""
    print("\n" + "="*60)
    print("    🚗 GEARGENIE VEHICLE SERVICE ADVISOR 🚗")
    print("="*60)
    print("\nInitializing system...\n")
    
    # Initialize connections
    client, collection = initialize_connections()
    
    print("\n" + "="*60)
    print("System Ready! Type 'exit', 'quit', or 'stop' to end session.")
    print("="*60 + "\n")
    ...
    while True:
        try:
            ...
            if user_input.lower() in ["exit", "quit", "stop", "bye", "goodbye"]:
                print("\n" + "="*60)
                print("GearGenie: Thank you for using GearGenie! Drive safely,")
                print("and don't hesitate to return if you need any assistance.")
                print("Have a great day! 🚗✨")
                print("="*60 + "\n")
                break
            
            # Search database for relevant context
            database_context, has_results = get_context_from_mongo(collection, user_input)
            ...
            messages = [
                ...
                *chat_history[-8:],  # Keep last 8 messages for context (4 exchanges)
